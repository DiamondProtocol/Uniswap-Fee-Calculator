...
def fetchData(configData,subgraphURL, startTime=1652000000, endTime=1652089128, force=True):
    ...
    if force:
        ...
        assert os.path.exists(rawDataFilePath), f'{rawDataFilePath} already existing.'
    f = open(rawDataFilePath, "a")
    totalCount = 0
    while cousorTime < endTime:
        dpayload = '{"query":"{\\n swaps(first: 1000, orderBy: timestamp, orderDirection: asc, where:' + '{ pool: \\"' + pool + '\\", '
        ...
        if response.status_code == 200:
            ...
            print(f'${cousorTime} response.status_code: ${response.status_code}, will retry')
    f.close()
...
def Raw2CSV(configData,switch=False):
    ...
    if len(lines) != 0:
        ...
        header = ["pool", "block_number", "block_hash", "block_time", "log_index", "tx_hash", "sender", "recipient", "amount0", "amount1", "sqrt_price_x96", "liquidity", "price", "tick"]
...
def getEarnedFee(prevTick,curTick,tickRange,feeTier): # TickRange = something like [100,200]
    if curTick > tickRange[0] and curTick < tickRange[1]: # In range 
        earnedFee = abs(curTick - prevTick) / (tickRange[1] - tickRange[0]) * feeTier # ROI made from the swap  
...
def getFeeAPR(configData,tickNumber):
    ...
    data = data.sort_values(['block_number', 'log_index'], ascending = [True, True])
    ...
    for index,value in data.iterrows():
        if noPosition: # initialize tickRange 
            tickRange = [value['tick'] - tickNumber, value['tick'] + tickNumber]
            noPosition = False 
        else: 
            earnedFee = getEarnedFee(value['prevTick'], value['tick'],tickRange,feeTier)
            culmulativeFee += earnedFee 
    
    backtestDays = (data.iloc[-1]['block_time'] - data.iloc[0]['block_time']) / 86400 # days 
    
    feeAPR = culmulativeFee * 365 / backtestDays * 100 # calculate Fee APR (%) based on tickNumber liquidity 
